...
@section('content')
    <form method="POST" action="{{ route('register') }}" class="flex flex-col items-center justify-center min-h-full">
        @csrf
        @if (session('status'))
            <div
        ...
        <div class="!w-1/5 !mb-0 card-holder">
            <div class="flex flex-col w-full pb-6">
                <x-forms.input name="username" :label="__('Username')" :value="old('username')" :error="$errors" focus auto required
                    nolbl />
